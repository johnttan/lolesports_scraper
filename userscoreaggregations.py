    ...
    for user in users:
            ...
            if playername in user['newplayersfirstgame'].keys():
                for game in player['gamesplayed']:
                    if game['gameID'] >= user['newplayersfirstgame'][playername]:
                        # print(totaluserscore, game['gameID'], player['playername'])
        ...
        for player in user['oldroster']:
            if player is not None:
                for game in player['gamesplayed']:
                    if player['playername'] in user['newplayersfirstgame'].keys():
                        if game['gameID'] <= user['oldplayerslastgame'][player['playername']] and game['gameID'] >= user['newplayersfirstgame'][player['playername']]:
                            # print(totaluserscore, game['gameID'], player['playername'])
                            totaluserscore += game['players'][player['playername']]['score']['totalscore']
                    else:
                        if game['gameID'] <= user['oldplayerslastgame'][player['playername']]:
...
# cyanide = cPlayers.find_one({'playername':'Cyanide'})
# vander = cPlayers.find_one({'playername':'VandeR'})
...
# cUsers.update({'username':'Arcwelder'}, {'$set':{'oldplayerslastgame':{'Nyph':2015, 'VandeR':2072, 'Cyanide':2072}, 'newplayersfirstgame':{'VandeR':2019, 'Amazing': 2020, 'Aphromoo':1941 }, 'oldroster':[nyph, cyanide, vander]}})
